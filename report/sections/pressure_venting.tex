...
\subsection{Transition between choked and unchoked flow}

For compressible discharge through the vent, the flow becomes choked
when the downstream-to-upstream pressure ratio satisfies
...
Defining the critical pressure ratio as
...
the corresponding minimum upstream pressure for choked discharge is
...
\begin{equation}
    ...
    \label{eq:critical_upstream_pressure}
\end{equation}

Immediately after vent opening, the internal pressure may therefore be
sufficiently high for the discharge to be choked. As the cell
depressurizes, the upstream pressure eventually falls below
$P_{\mathrm{up,crit}}$, after which the model transitions to the
unchoked-flow expression.

No separate state variable is required for this transition. The flow
regime is determined directly from the instantaneous upstream and
